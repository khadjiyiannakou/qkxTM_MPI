...
static void qkxTM_swap_4(float *Rd, size_t N)
{
  register char *i,*j,*k;
  char swap;
  char *max;
...
int qkxTM_MPI_getVectorLime(char *fname, Vector &v){
...
//lime_type="scidac-binary-data";
            if((strcmp(lime_type,"quda-propagator-format")==0) || (strcmp(lime_type,"quda-source-format")==0))
            {
	      lime_data_size = limeReaderBytes(limereader);
	      lime_data = (char * )malloc(lime_data_size);
	      limeReaderReadData((void *)lime_data,&lime_data_size, limereader);
	      sscanf(qkxTM_getParam("<precision>",lime_data, lime_data_size),"%i",&isDouble);    
   ...
   if(isDouble)
   {
	...
	qkxTM_swap_8((double*) buffer,2*4*3*v.lV4d);
      i=0;
      for(t=0; t<v.lL[3];t++)
      for(z=0; z<v.lL[2];z++)
      for(y=0; y<v.lL[1];y++)
	...
	fprintf(stderr,"process %i: Error in %s! Out of memory\n",v.rank, __func__);
      ...
      if(!qkxTM_isBigEndian())      
	qkxTM_swap_4((float*) buffer,2*4*3*v.lV4d);
      i=0;
      for(t=0; t<v.lL[3];t++)
      for(z=0; z<v.lL[2];z++)
      for(y=0; y<v.lL[1];y++)
	  ...
	  for(c1=0; c1<3; c1++)
	    {
	      ...
	      v.M[LEXIC(t,z,y,x,v.lL)][mu][c1].imag() = ((float*)buffer)[i+1];
	      i+=2;
	    }
      free(buffer);
      MPI_File_close(&mpifid);
...
int qkxTM_MPI_getPropLime(char *fname , Propagator &prop , LatticeInfo *latInfo ){
  
  char tempFilename[257];
  Vector *v = new Vector(latInfo);

 for(int nu = 0 ; nu < 4 ; nu++)
    for(int c2 = 0 ; c2 < 3 ; c2++){
      int i = nu*3 + c2;
      sprintf(tempFilename,"%s.%04d",fname,i);
      qkxTM_MPI_getVectorLime(tempFilename,*v);
      
      for(int t = 0 ; t < prop.lL[3] ; t++)
	for(int z = 0 ; z < prop.lL[2] ; z++)
	  for(int y = 0 ; y < prop.lL[1] ; y++)
	    for(int x = 0 ; x < prop.lL[0] ; x++)
	      for(int mu = 0 ; mu < 4 ; mu++)
		for(int c1 = 0 ; c1 < 3 ; c1++)
		  prop.M[LEXIC(t,z,y,x,prop.lL)][mu][nu][c1][c2] = v->M[LEXIC(t,z,y,x,prop.lL)][mu][c1];
